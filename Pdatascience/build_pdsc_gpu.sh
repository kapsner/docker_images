...
docker build --network host -f image_pdsc_gpu/Dockerfile.multi -t pdsc_gpu_multistage .
...
docker build --network host --build-arg BASEIMAGE=pdsc_gpu:latest -f image_pdsc_jpylab/Dockerfile -t pdsc_jpylab_gpu .
