...
docker build --network host --build-arg BASEIMAGE=base_image:latest -f image_rdsc_base/Dockerfile -t rdsc_base .

printf "\nBuild rdsc_headless image\n"
#docker build -f image_rdsc_headless/Dockerfile -t rdsc_headless .
docker build --network host -f image_rdsc_headless/Dockerfile -t rdsc_headless .
